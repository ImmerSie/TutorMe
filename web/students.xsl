...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:param name="stuPassword"/>
    <xsl:param name="stuBirthday"/>
    ...
    <xsl:template match="student">
        <xsl:variable name="thisEmail"> <xsl:value-of select="status"/></xsl:variable>
        <xsl:choose>
            <xsl:when test="$stuEmail=$thisEmail">
                 <form action="account.jsp" method="POST">
                    <table>
                        <tr> 
                            <td>Full Name:</td> 
                            <td><input type="text" name="name" value="<xsl:value-of select="name"/>"/></td> 
                        </tr>
                        <tr> 
                            <td>Email:</td> 
